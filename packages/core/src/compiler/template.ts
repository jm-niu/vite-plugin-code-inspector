...
export interface CompileSFCTemplateOptions {
  code: string;
  id: string;
  type: "template" | "jsx";
  hideJsxTags?: string[];
          ...
          enter({ node }) {
            if (node.type === "JSXElement") {
              ...
              if (
                ...
                tagName.object.name === "React" &&
                tagName.property.name === "Fragment"
              )
                return;

              if (hideJsxTags.length) {
                const getTagName = (node: any): string => {
                ...
                if (hideJsxTags.includes(tagNameStr)) return;
              }
