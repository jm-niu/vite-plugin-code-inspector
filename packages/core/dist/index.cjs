...
function parseFileLocation(urlPath) {
...
async function openInEditor(file, line, column, editor) {
  try {
...
function setupOpenInEditorMiddleware(server, editor) {
...
function getInspectorPath() {
  const pluginPath = (0, import_vite2.normalizePath)(
...
function normalizeComboKeyPrint(toggleComboKey) {
  return toggleComboKey.split("-").map(
    (key) => toggleComboKeysMap[key] || key[0].toUpperCase() + key.slice(1)
  ).join((0, import_kolorist.dim)("+"));
}
...
import 'virtual:dev-inspector-path:load.js'`
      ...
      configureServer(server) {
        ...
        const { toggleComboKey, launchEditor } = normalizedOptions;
        setupOpenInEditorMiddleware(server, launchEditor);
        toggleComboKey && (server.printUrls = () => {
          const keys = normalizeComboKeyPrint(toggleComboKey);
          _printUrls();
          console.log(
            `  ${(0, import_kolorist.green)("\u279C")}  ${(0, import_kolorist.bold)("Vite Inspector")}: ${(0, import_kolorist.green)(
              `Press ${(0, import_kolorist.yellow)(keys)} in App to toggle the Inspector`
            )}
`
          );
        });
    ...
    {
      name: "vite-plugin-code-inspector:post",
      enforce: "post",
      apply(_, { command }) {
      ...
      transform(code) {
        ...
        s.appendLeft(
          0,
          `/* Injection by vite-plugin-code-inspector Start */
